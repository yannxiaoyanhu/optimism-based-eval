...
4. For FID-based evaluation, run [FID-eval.py](https://github.com/yannxiaoyanhu/optimism-based-eval/blob/main/FID-eval.py). To reproduce the results, we use a batch size of 5 and set the parameter *ths_ratio* in line 24, which is used to truncate the covariance matrix for the generated images, to be 0.005 for InceptionV3.Net and 0.05 for both dinov2 and CLIP, respectively (details can be found in part 5 of Appendix D in the paper). For IS-based evaluation, run [IS-eval.py](https://github.com/yannxiaoyanhu/optimism-based-eval/blob/main/IS-eval.py).
