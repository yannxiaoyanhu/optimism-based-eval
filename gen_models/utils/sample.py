...
import gen_models.utils.losses as losses
...
def generate_images(z_prior, truncation_factor, batch_size, z_dim, num_classes, y_sampler, radius, generator, discriminator,
    ...
    if is_train:
        ...
        if truncation_factor != -1:
            if is_stylegan:
                assert 0 <= truncation_factor <= 1, "Stylegan truncation_factor must lie btw 0(strong truncation) ~ " \
                                                    "1(no truncation)"
            else:
...
def stylegan_generate_images(zs, fake_labels, num_classes, style_mixing_p, update_emas,
    ...
    if truncation_psi == -1:
        ...
        ws = generator_mapping(zs, one_hot_fake_labels, truncation_psi=truncation_psi,
                               truncation_cutoff=truncation_cutoff,
                               update_emas=update_emas)
    if style_mixing_p > 0:
        cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
        cutoff = torch.where(torch.rand([], device=ws.device) < style_mixing_p, cutoff,
...
def burn_in_sampling(gen, gen_dim, burn_in_n, f_dim, num_classes, inception_model, device, save_imgs=False, path=None):
    batch = min(100, burn_in_n)
    pred_arr = np.empty((burn_in_n, f_dim))
    start_idx = 0
    idx = 0
    for _ in range(burn_in_n // batch):
        with torch.no_grad():
            fake_imgs = gen(z=sample_normal(batch_size=batch, z_dim=gen_dim, truncation_factor=1.0, device=device),
                            label=torch.randint(low=0, high=num_classes, size=(batch, ),
                                                dtype=torch.long, device=device),
                            eval=True)
            if save_imgs:
                os.makedirs(path, exist_ok=True)
                for sample in fake_imgs:
                    save_image((sample + 1) * 0.5, os.path.join(path, "{}.png".format(idx)))
                    idx += 1

            pred = inception_model((fake_imgs + 1) * 0.5)[0]
