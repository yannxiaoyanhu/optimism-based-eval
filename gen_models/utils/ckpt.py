    ...
    if load_model:
        if is_freezeD:
            mismatch_names = misc.load_parameters(src=ckpt["state_dict"],
                                                  ...
                                                  strict=False)
            print("The following parameters/buffers do not match with the ones of the pre-trained feat_model:", mismatch_names)
        else:
...
def load_StudioGAN_ckpts(ckpt_dir, load_best, Gen, Dis, g_optimizer, d_optimizer, run_name, apply_g_ema, Gen_ema, ema,
    ...
    x = join(ckpt_dir, "feat_model=G-{when}-weights-step=".format(when=when))
    Gen_ckpt_path = glob.glob(glob.escape(x) + '*.pth')[0]
    y = join(ckpt_dir, "feat_model=D-{when}-weights-step=".format(when=when))
    ...
    if apply_g_ema:
        z = join(ckpt_dir, "feat_model=G_ema-{when}-weights-step=".format(when=when))
...
def load_best_model(ckpt_dir, Gen, Dis, apply_g_ema, Gen_ema, ema):
    ...
    Gen_ckpt_path = glob.glob(join(ckpt_dir, "feat_model=G-best-weights-step*.pth"))[0]
    Dis_ckpt_path = glob.glob(join(ckpt_dir, "feat_model=D-best-weights-step*.pth"))[0]
    ...
    if apply_g_ema:
        Gen_ema_ckpt_path = glob.glob(join(ckpt_dir, "feat_model=G_ema-best-weights-step*.pth"))[0]
        load_ckpt(model=Gen_ema,
                  optimizer=None,
                  ckpt_path=Gen_ema_ckpt_path,
                  load_model=True,
...
def check_is_pre_trained_model(ckpt_dir, GAN_train, GAN_test):
    ...
    ckpt_list = glob.glob(join(ckpt_dir, "feat_model=C-{mode}-best-weights.pth".format(mode=mode)))
    if len(ckpt_list) == 0:
...
def load_GAN_train_test_model(model, mode, optimizer, RUN):
    ckpt_path = join(RUN.ckpt_dir, "feat_model=C-{mode}-best-weights.pth".format(mode=mode))
    ckpt = torch.load(ckpt_path, map_location=lambda storage, loc: storage)
