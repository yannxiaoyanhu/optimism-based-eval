...
import torch
import gen_models.utils.style_misc as misc

from gen_models.utils.style_ops import conv2d_gradfix, bias_act, filtered_lrelu
...
class Generator(torch.nn.Module):
    def __init__(self, z_dim, c_dim, w_dim, img_resolution, img_channels, info_num_discrete_c, info_dim_discrete_c,
                 info_num_conti_c, info_type, mapping_kwargs={}, synthesis_kwargs={}):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
        self.w_dim = w_dim
        ...
        if info_type in ["discrete", "both"]:
            z_extra_dim += info_num_discrete_c * info_dim_discrete_c
        if info_type in ["continuous", "both"]:
            z_extra_dim += info_num_conti_c
    ...
    def forward(self, z, label, eval=False, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, label, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
